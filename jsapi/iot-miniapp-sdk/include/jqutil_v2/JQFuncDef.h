...
public:
    ...
    inline void Set(const std::vector<uint8_t> &msg) { jq_set_value(_ctx, &_value, JS_NewArrayBufferCopy(_ctx, msg.data(), msg.size())); }
    inline void Set(const Bson &msg) { jq_set_value(_ctx, &_value, bsonToJSValue(_ctx, msg)); }
    ...
    void ThrowSyntaxError(const char* fmt, ...);
    void ThrowTypeError(const char* fmt, ...);
    void ThrowReferenceError(const char* fmt, ...);
    void ThrowRangeError(const char* fmt, ...);
...
public:
    ...
    const JQObjectProperty* getProperty() const;
...
protected:
    ...
    const JQObjectProperty *_property=NULL;
};
...
public:
    ...
    bool hasData() const { return _data != NULL; }
    template<typename T>
...
public:
    ...
    const JQObjectProperty& property() const { return *_property; }
