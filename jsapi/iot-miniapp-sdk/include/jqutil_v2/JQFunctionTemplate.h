...
private:
    ...
    JQFunctionTemplate(JQTemplateEnvRef env, const std::string &name);

public:
    static JQFunctionTemplateRef New(JQTemplateEnvRef env, const std::string &name);
    ...
    {
        JSAtom prop1 = JS_NewAtom(_tplCtx, prop);
        JQObjectProperty& p = PrototypeTemplate()->Set(prop1, [func](JQFunctionInfo& info) {
          T* obj = info.UnwrapThis<T>();
          assert(obj != NULL);
          (obj->*func)(info);
        });
    ...
    JQFunctionTemplateRef clone() const;
