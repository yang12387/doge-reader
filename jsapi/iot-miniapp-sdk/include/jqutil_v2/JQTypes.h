...
public:
    ...
    inline JSValue dupValue() { return JS_DupValue(_ctx, _value); }
    inline void setValue(JSValueConst val)
    { if (_scope) jq_set_value(_ctx, &_value, val);
        else _value = val; }
    inline bool isUndefined() { return JS_IsUndefined(_value); }
    ...
    inline bool isException() { return JS_IsException(_value); }
    inline bool isArrayBufferView() { return JQ_IsABOrABView(_ctx, _value); }
...
class JQAtom {
public:
    JQAtom(JSContext *ctx, JSAtom atom): _ctx(ctx), _atom(atom), _scope(false) {}
    JQAtom(JSContext *ctx, const std::string &name): _ctx(ctx)
    {
        _atom = JS_NewAtom(ctx, name.c_str());
        _scope = true;
    }
    ~JQAtom() {
        if (_scope) {
    ...
    JQAtom(const JQAtom &rval)
    {
        ...
        _atom = rval._atom;
        if (_scope) {
            JS_DupAtom(_ctx, _atom);
        }
    }

    JSAtom getAtom() const
    {
        return _atom;
    }
...
class JQAtomScope: public JQAtom {
public:
    JQAtomScope(JSContext *ctx, JSAtom atom)
            :JQAtom(ctx, atom) { JQAtom::_setScope(true); }
...
using JQValueScope = JQScopeValue<JQValue>;
